      ...
      <div className="card flex-1 overflow-y-auto space-y-4 !p-4">
        {msgs.map((m, i) => (
          <div key={i} className={`flex gap-2.5 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-[78%] rounded-2xl px-4 py-2.5 text-[15px] break-words shadow-sm ${
              m.role === 'user'
                ? 'bg-gradient-to-br from-brand-500 to-brand-600 text-white rounded-br-sm'
                : 'bg-white/75 backdrop-blur-sm border border-white/60 text-slate-800 rounded-bl-sm'}`}>
              {m.role === 'bot'
        ...
        {busy && (
          <div className="flex gap-2.5 justify-start">
            <div className="shrink-0 h-8 w-8 rounded-full bg-brand-100 flex items-center justify-center text-lg">🌱</div>
            <div className="bg-white/75 backdrop-blur-sm border border-white/60 rounded-2xl rounded-bl-sm px-4 py-3 flex items-center gap-1">
